...
const ascending = currify((attr, a, b) => a[attr] - b[attr]);

const cmp = (attr, order) => {
    if (order === 'desc')
        return not(ascending(attr));
    
    return ascending(attr);
};

const sortFiles = currify((attr, order, array) => {
    return array.sort(cmp(attr, order));
});
...
function readify(path, options, fn) {
    ...
    check(path, type, fn);
    checkSort(options.order, options.sort);
    
    readdir(path)
        .then(sortFiles(options.sort, options.order))
        .then(changeOrder)
